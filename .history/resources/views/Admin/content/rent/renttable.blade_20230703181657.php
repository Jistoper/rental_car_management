...
<script>
    function calculateTotalPrice() {
      ...
      document.getElementById('total_price')[0].value = totalPrice.toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    }
</script>
                ...
                <div class="modal-body">
                    <form action="{{ route('car.rentStore', $cars['car_id']) }}" method="POST" class="row g-3 needs-validation" novalidate>
                        ...
                        <div class="text-center mb-3">
                            <input type="button" class="btn btn-primary rounded-pill" onclick="calculateTotalPrice()">
                            <span>
                                Test
                            </span>
                        </div>
